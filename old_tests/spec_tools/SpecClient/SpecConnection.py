...
class SpecConnectionDispatcher(asyncore.dispatcher):
    ...
    def checkServer(self):
        ...
        while not self.scanport or self.port < MAX_PORT:
            ...
            s.settimeout(0.2)
            try:
                #dprint was causing issues so it was disabled at initalization... LAZY FIX
                print "attempted to use dprint which was disabled -  IF YOU SEE THIS FIX IT"
                #dprint("trying to connect to %s / %s" % (self.host,self.port))
    ...
    def registerChannel(self, chanName, receiverSlot, registrationFlag = SpecChannel.DOREG, dispatchMode = SpecEventsDispatcher.UPDATEVALUE):
        ...
        #dprint("registered channels are %s " % str(listreg))
