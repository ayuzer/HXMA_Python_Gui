...
class Plotter(Qwt.QwtPlot):
    ...
    def handle_vbar_position(self, pv_name):
        value = self.monitor.get_value(pv_name)
    ...
    def mouseMoveEvent(self, event):
        ...
        if self._dragBar:
            self.monitor.update(VAR.VERT_BAR_POS_X, xFloat)
    ...
    def mousePressEvent(self, event):
        ...
        cur_vbar_value = self.monitor.get_value(VAR.VERT_BAR_POS_X)
    ...
    def vertical_bar(self, x_pos):

        x = [x_pos, x_pos]
        y = [-1, 1]

        self.vbar.setData(x, y)
        self.replot()
...
class MainWindow(QtGui.QMainWindow, UiMixin, DragTextMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pushButton_popup.clicked.connect(self.handle_pushButton_popup)
        ...
        self.lineEdit_example.editingFinished.connect(self.handle_lineEdit_changed)
        self.lineEdit_example.setToolTip("this is the tool tip")

        # Set up graph
        self._graph = Plotter(monitor=self.monitor)
        self.verticalLayout_plot_container.addWidget(self._graph)
        ...
        for v in xrange(1000):
            x = math.pi * (float(v)/200.0)
            if x > max_x: max_x = x
            if x < min_x: min_x = x
            y = math.cos(x)
            data.append((x, y))

        self._graph.new_plot(data)

        # self._graph.vertical_bar(0)

        self.monitor.start()

        v_bar_x_pos = min_x + (max_x - min_x) / 2.0

        self.monitor.update(VAR.VERT_BAR_POS_X, v_bar_x_pos)
    ...
    def handle_pushButton_popup(self):

        center_point = self.geometry().center()
        window = Popup(monitor=self.monitor, center=center_point)
